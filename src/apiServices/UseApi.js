...
const useApi = ({ method = 'GET', url = '', manual = false }) => {
  console.log('hello123456789')
  const [response, setResponse] = useState(null);
  const [error, setError] = useState(null);
  const [loading, setLoading] = useState(false);

  const { apiKey, apiEndPoint } = useSelector(state => state.login);
  ...
  const callApi = useCallback(
    ...
    async (body = null, customUrl = null, customHeaders = {}) => {
      ...
      const BASE_URL =  apiEndPoint || PartnerBaseUrl() ;
      ...
      try {
        ...
        const errMsg = err?.response?.data || err?.message || 'Something went wrong';
